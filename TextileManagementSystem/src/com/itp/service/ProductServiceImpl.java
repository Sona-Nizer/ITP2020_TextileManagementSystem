...
public class ProductServiceImpl implements iProductService {
	...
	public int editProduct(Product p1) {
		...
		try {
			String query1 = "UPDATE textilemanagementsystem.product SET name=?, "
					+ "category=?, qty=?, size=?, price=?, reOrderLevel=? WHERE ID=?";
			
			PreparedStatement ps = con.prepareStatement(query1);

			ps.setString(1, p1.getProductName());
			ps.setString(2, p1.getProductCategory());
			ps.setInt(3, p1.getProductQty());
			ps.setString(4, p1.getProductSize());
			ps.setDouble(5, p1.getProductPrice());
			ps.setInt(6, p1.getProductReOrderLevel());
			ps.setString(7, p1.getProductID());
